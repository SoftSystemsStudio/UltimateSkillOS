...
# Global registry instance (suitable for single-process use)
registry: CircuitBreakerRegistry = CircuitBreakerRegistry()


# Optional Redis-backed implementation for multi-process persistence
try:
    import redis
    ...
    # If environment requests Redis, override global registry
    _redis_url = os.environ.get("SKILLOS_CIRCUIT_REDIS_URL")
    if _redis_url:
        try:
            registry = RedisCircuitBreakerRegistry(_redis_url)
        except Exception:
            # Fall back to in-memory registry on any failure
